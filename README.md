...
> **只读屏 + 只复制**：DraftMate 不模拟键鼠、不自动发送。截图读出对话、在右侧给几条候选回复，你点「复制」再自己粘贴到聊天框——把账号风险降到最低，也让你对每一句话有最终决定权。
...
**授权**:DraftMate 只截图、不模拟键鼠,所以一般只需在 系统设置 → 隐私与安全性 → **屏幕录制** 里勾上你运行它的**终端**(或打包后的 `DraftMate.app`),之后重启它生效。(仅当 Quartz 定位不到窗口、回退到 AppleScript 取窗口位置时,才会另需「辅助功能」。)
