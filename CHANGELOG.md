...
- **蒸馏在真实长噪声数据上泛化/幻觉**:7B 把口头梗当实体("冷知识"当游戏)、一次性提及当爱好(原神)、个别 sender 认错。对策=蒸馏 prompt 强制每条附原文引用 `[据:"原话"]`、编不出引用不准写、禁常识联想;固化后主干忠实、可逐条核对,残留个别瑕疵属 7B 上限(升级 14b/claude 更干净,路已留)。
